/**
 * @file <sdfileinput> This is my cool script
...
function createFileList() {
    ...
    var files = Array.prototype.concat.apply([], arguments);
    var i = 0;
...
function delItem(el) {
    ...
    if (fInp.files.length > 0) {
        el.parentNode.parentNode.parentNode.querySelector('.sd-file-title').innerHTML = `${fInp.files.length} File Selected`
    } else {
        el.parentNode.parentNode.parentNode.querySelector('.sd-file-title').innerHTML = 'No File Chosen'
    }
...
function handleFile(e) {
    ...
    // set File list view
    if (e.target.files.length > 0) {
        e.target.parentNode.querySelector('.sd-file-title').innerHTML = e.target.files.length + ' File Selected'
        e.target.parentNode.parentNode.querySelector('.sd-files').innerHTML = ''
        for (let i = 0; i < e.target.files.length; i += 1) {
            let img = null
            ...
            e.target.parentNode.parentNode.querySelector('.sd-files').insertAdjacentHTML('beforeend', `<div>
    ...
    if (err.length > 0) {
        for (let i = 0; i < err.length; i += 1) {
            e.target.parentNode.parentNode.querySelector('.sd-files').insertAdjacentHTML('afterbegin', `
            <div style="background: #fff2f2;color: darkred;display:none" class="sd-file-err">
        ...
        const errNods = e.target.parentNode.parentNode.querySelectorAll('.sd-files>.sd-file-err')
